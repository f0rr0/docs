...
## AWS RDS Proxy & AWS Aurora Data API

At present, Prisma is not compatible with either RDS Proxy or Aurora Serverless Data API. Both these components enable FaaS applications to reply on AWS managed connection pooling to achieve reliable connection reuse. Support for these is being tracked here:
